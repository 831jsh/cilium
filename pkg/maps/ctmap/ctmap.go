...
func doGC6(m *bpf.Map, filter *GCFilter) int {
	...
	for {
		...
		if err != nil {
			...
			if prevKeyValid {
				currentKey = prevKey
				// Restart from a given previous key only once
				prevKeyValid = false
			} else {
				// No previous key or restart from the previous key failed as well, restarting from
				// the beginning of the map
				restarts++
				if restarts > 2 {
					// TODO Add a metric we can bump and observe here.
					log.WithError(err).Error("Garbage collection on IPv6 CT map failed to finish")
					break
				}
			}
			continue
	...
	for {
		...
		if err != nil {
			...
			if prevKeyValid {
				...
				if restarts > 2 {
					// TODO Add a metric we can bump and observe here.
					log.WithError(err).Error("Garbage collection on IPv4 CT map failed to finish")
					break
				}
			}
			continue
